...
class Entry:
    ...
    def __init__(self, src, idx, fr=0, sz=sys.maxsize):
    ...
    def getAcc(self):
        ...
        if not match:
            ...
            raise Exception(
                'Unable to match capturing group "key" in regex: ' + self.__src.regx.pattern)  # FIXME: Raise a better error.
...
class SwissProtFile:
    ...
    def __init__(self, index_name, regx_string=''):
        ...
        self.__mod = pickle.load(idx)  # Last modified
        self.__src = SwissProtFile.SwissProtFileHandle(pickle.load(idx), regx)  # Source file.
        idxs = pickle.load(idx)  # The indexes of each entry.
        idx.close()
    ...
    def items(self):
        """ Cost: O(n) """
        return [(key, self.__lst[val]) for key, val in self.__dct.items()]

    def keys(self):
        return list(self.__dct.keys())
...
def index_file(filename, index_name=None):
    ...
    if os.path.exists(index_name):
        try:
            ...
            iMod = pickle.load(infile)  # Indexed modification time.
            infile.close()
            ...
            if iMod == cMod:
                return
        except EOFError as e:  # If the file is empty then continue anyway.
            pass
    ...
    pickle.dump(os.stat(filename)[stat.ST_MTIME], outfile, pickle.HIGHEST_PROTOCOL)
    pickle.dump(filename, outfile, pickle.HIGHEST_PROTOCOL)
    pickle.dump(lst, outfile, pickle.HIGHEST_PROTOCOL)
    outfile.close()
