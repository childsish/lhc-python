...
class BpmapFile:
    def __init__(self, filename):
        ...
        self.__keys = pickle.load(infile)
        self.__idxs = pickle.load(infile)
        self.__counts = pickle.load(infile)
        infile.close()

    def __len__(self):
        total = 0
        for val in self.__counts.values():
            total += val
        return total
...
def indexBpmapFile(filename):
    ...
    pickle.dump(keys, outfile, pickle.HIGHEST_PROTOCOL)
    pickle.dump(idxs, outfile, pickle.HIGHEST_PROTOCOL)
    pickle.dump(counts, outfile, pickle.HIGHEST_PROTOCOL)
    outfile.close()
